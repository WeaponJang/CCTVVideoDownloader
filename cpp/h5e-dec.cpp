...
// OPTIMIZATION: Linear scan for EPB adjustment instead of binary search.
inline size_t decrypt_type5_new(uint8_t* nal, size_t len, uint32_t stride) {
    ...
    // OPTIMIZATION 1: Pre-allocate memory for EPB list to avoid reallocations.
    // Assuming max density EPB (00 00 03) ~ 1/10 of bytes for safety.
    epbs.reserve(len > 10 ? len / 10 : 4);

    for (size_t i = 0; i + 2 < len; i++) {
        if (nal[i] == 0 && nal[i + 1] == 0 && nal[i + 2] == 3) epbs.push_back(i);
    }

    // OPTIMIZATION 3: Use linear index tracking instead of binary search.
    // Since 'o' increases monotonically, we can scan 'epbs' linearly.
    size_t epb_idx = 0;
    size_t epb_count = epbs.size();

    for (size_t k = 0;; k++) {
        size_t o = 64 + k * (size_t)stride;
        if (o + 16 > len) break;

        // Advance epb_idx until we find the first EPB >= o
        while (epb_idx < epb_count && epbs[epb_idx] < o) {
            epb_idx++;
        }
        // The number of EPBs strictly before 'o' is now 'epb_idx'.
        size_t adj = epb_idx;
...
inline uint32_t type5_stride_from_nal(const uint8_t* nal, size_t len) {
    ...
    uint8_t key[6]; // key[0..3]=nal[5..8], key[4]=nal[9], key[5]=nal[10]
    key[0] = nal[5];
    key[1] = nal[6];
    key[2] = nal[7];
    key[3] = nal[8];
...
inline void collect_epb_positions(const uint8_t* nal, size_t len, std::vector<size_t>& epbs) {
    epbs.clear();
    if (!nal || len < 3) return;
...
// OPTIMIZATION 3: This function was O(N) inside a loop. In decrypt_type1_new below,
// we replace this call with linear index tracking to achieve O(N) total.
inline size_t epb_adj_before(const std::vector<size_t>& epbs, size_t o) {
    size_t n = 0;
    for (size_t e : epbs) if (e < o) n++; else break;
    return n;
}
...
// OPTIMIZATION 3: Replaced binary search/linear scan inside loop with index tracking.
inline size_t decrypt_type1_new(uint8_t* nal, size_t len, uint32_t stride = 511, size_t start = 64, size_t guard = 17) {
    if (!nal || stride < 4 || len < 3) return len;
    const uint8_t hdr[3] = {nal[0], nal[1], nal[2]};
    std::vector<size_t> epbs;
    collect_epb_positions(nal, len, epbs);

    // OPTIMIZATION 3: Linear index tracking.
    size_t epb_idx = 0;
    size_t epb_count = epbs.size();
    ...
    for (size_t k = 0;; k++) {
        ...
        // Advance epb_idx until epbs[epb_idx] >= o
        while (epb_idx < epb_count && epbs[epb_idx] < o) {
            epb_idx++;
        }
        size_t adj = epb_idx; // Current count of EPBs before o
...
inline size_t expand_af_steal(uint8_t* data, size_t len, size_t pkt_off, size_t need) {
    ...
    if (afc == 1) { // payload only -> introduce AF. af_len = need - 1 (see Python notes).
        if (need < 1) return 0;
...
inline size_t decrypt_ts_inplace(uint8_t* data, size_t len, Session& session, uint16_t vpid = 0x100) {
    ...
    // OPTIMIZATION 1: Pre-allocate capacity for TS packet storage.
    // Max spans = number of TS packets = len / 188.
    spans.reserve(len / 188);
    ...
    auto flush = [&]() {
        ...
        // OPTIMIZATION 1: Pre-allocate for NAL units. 
        // Rough estimate: 1 NAL per ~100 bytes on average.
        starts.reserve(es.size() / 100);

        size_t i = 0;
        while (i + 3 < es.size()) {
            if (i + 4 <= es.size() && es[i] == 0 && es[i + 1] == 0 && es[i + 2] == 0 && es[i + 3] == 1) {
        ...
        new_es.reserve(es.size()); // Pre-allocate output buffer
        size_t cursor = 0;
        for (size_t idx = 0; idx < starts.size(); idx++) {
            ...
            if (cursor < pos) new_es.insert(new_es.end(), es.begin() + (std::ptrdiff_t)cursor, es.begin() + (std::ptrdiff_t)pos);
            new_es.insert(new_es.end(), es.begin() + (std::ptrdiff_t)pos, es.begin() + (std::ptrdiff_t)(pos + sc));
            if (pos + sc >= end) {
                cursor = end;
                continue;
            }
            size_t nal_len = end - (pos + sc);
            std::vector<uint8_t> nal(es.begin() + (std::ptrdiff_t)(pos + sc), es.begin() + (std::ptrdiff_t)end);
        ...
        if (cursor < es.size()) new_es.insert(new_es.end(), es.begin() + (std::ptrdiff_t)cursor, es.end());
        ...
        if (capacity > new_pes.size()) {
            ...
            new_spans.reserve(spans.size()); // Preserve capacity
            for (auto& sp : spans) {
        ...
        for (auto& sp : spans) {
            ...
            if (chunk > 0) std::memcpy(data + pkt_off + pi, new_pes.data() + off, chunk);
            if (chunk < pl) {
...
inline std::vector<uint8_t> decrypt_ts(const uint8_t* data, size_t len, Session& session, uint16_t vpid = 0x100) {
...
inline std::vector<uint8_t> decrypt_ts_default(const uint8_t* data, size_t len, uint16_t vpid = 0x100) {
    Session session;
...
const char* cctv_h5e_version(void) {
    return "cctv_h5e by pure cpp 1.0 (2026-07-25)";
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::fprintf(stderr, "cctv_h5e --- pure C++ hls_h5e TS decrypt\n"
                             " %s\n"
    ...
    for (int i = 3; i < argc; i++) {
        if (std::strcmp(argv[i], "--pid") == 0 && i + 1 < argc) // Optimization: use strcmp
